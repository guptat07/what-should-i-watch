...
// 4. ONLY ADD FILTERS OR CONSTRAINTS IF EXPLICITLY REQUESTED OR DIRECTLY IMPLIED BY USER.
//    - Example: Only return with_watch_monetization_types: "flatrate, rent, buy" if a query mentions purchasing a film.
//    - Example: Only add quality filters like vote averages or vote counts IF the user explicitly asks for ratings, scores, or popularity metrics.
//    - Infer, don't guess. Stick to DIRECT implications
//    - Example: "Old Hollywood" does not necessarily imply "classic", which does not necessarily imply a certain vote average. Even if "classic" did imply a certain vote average, that's an indirect result since it doesn't come immediately from "Old Hollywood", so it is incorrect.
//    - ONLY RETURN PROPERTIES IF THEY HAVE VALUES; OMIT EMPTY PROPERTIES.
//    - An omitted property is a completely valid and often correct response. NEVER output keys with null, empty strings "", or guessed default values.